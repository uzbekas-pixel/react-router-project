...
// Demo ma'lumotlar (Firestore bo'sh bo'lsa)
const DEMO_USERS = [
  { id:"d1", displayName:"Sardor N.",   email:"sardor@mail.com",   xp:4850, courses:8, quizAvg:96, streak:21, avatar:null },
  { id:"d2", displayName:"Malika E.",   email:"malika@mail.com",   xp:4200, courses:7, quizAvg:91, streak:18, avatar:null },
  { id:"d3", displayName:"Bobur Y.",    email:"bobur@mail.com",    xp:3750, courses:6, quizAvg:88, streak:15, avatar:null },
  { id:"d4", displayName:"Nilufar K.",  email:"nilufar@mail.com",  xp:3100, courses:5, quizAvg:84, streak:12, avatar:null },
  { id:"d5", displayName:"Jasur T.",    email:"jasur@mail.com",    xp:2800, courses:4, quizAvg:79, streak:9,  avatar:null },
  { id:"d6", displayName:"Kamola R.",   email:"kamola@mail.com",   xp:2400, courses:4, quizAvg:76, streak:7,  avatar:null },
  { id:"d7", displayName:"Sherzod M.",  email:"sherzod@mail.com",  xp:2100, courses:3, quizAvg:72, streak:6,  avatar:null },
  { id:"d8", displayName:"Dilorom S.",  email:"dilorom@mail.com",  xp:1800, courses:3, quizAvg:68, streak:5,  avatar:null },
  { id:"d9", displayName:"Alisher H.",  email:"alisher@mail.com",  xp:1500, courses:2, quizAvg:65, streak:4,  avatar:null },
  { id:"d10",displayName:"Kamol R.",    email:"kamol@mail.com",    xp:1200, courses:2, quizAvg:60, streak:3,  avatar:null },
];
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:"50%", background:color, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:size*0.38, flexShrink:0, overflow:"hidden" }}>
      {user.photoURL || user.avatar
        ? <img src={user.photoURL || user.avatar} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
        : initials}
    </div>
  );
};
...
const Leaderboard = ({ darkMode }) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "users"), (snap) => {
      ...
      if (list.length > 2) {
        // Firestore da foydalanuvchilar bo'lsa ularni ishlatamiz
        // stats bo'lmasa demo qiymat qo'shamiz
        const enriched = list.map((u) => ({
          ...u,
          xp:       u.xp       || Math.floor(Math.random() * 3000 + 500),
          courses:  u.courses  || Math.floor(Math.random() * 6 + 1),
          quizAvg:  u.quizAvg  || Math.floor(Math.random() * 40 + 55),
          streak:   u.streak   || Math.floor(Math.random() * 15 + 1),
        }));
        setUsers(enriched);
      }
      setLoading(false);
  ...
  const sorted = [...users].sort((a, b) => (b[activeTab] || 0) - (a[activeTab] || 0));
    ...
    <div style={{ width:"100%", maxWidth:800, margin:"0 auto", padding:"40px 16px 80px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12, marginBottom:28 }}>
          ...
          <div style={{ display:"flex", gap:6, background:darkMode?"#1e293b":"#f1f5f9", borderRadius:10, padding:4 }}>
            {[{id:"all",label:"Barchasi"},{id:"month",label:"Oy"},{id:"week",label:"Hafta"}].map((p)=>(
              <button key={p.id} onClick={()=>setPeriod(p.id)} style={{ padding:"6px 14px", borderRadius:8, border:"none", cursor:"pointer", background:period===p.id?"#3b82f6":"transparent", color:period===p.id?"#fff":darkMode?"#94a3b8":"#374151", fontSize:12, fontWeight:600, transition:"all 0.2s" }}>
                {p.label}
              </button>
            ))}
          </div>
        ...
        {sorted.length >= 3 && (
          <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"center", gap:12, marginBottom:28 }}>
            ...
            <div style={{ textAlign:"center", flex:1 }}>
              ...
              <p style={{ fontWeight:700, fontSize:13, color:darkMode?"#f1f5f9":"#111", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:80, margin:"4px auto 2px" }}>{sorted[1].displayName||sorted[1].email?.split("@")[0]}</p>
            ...
            <div style={{ textAlign:"center", flex:1 }}>
              ...
              <p style={{ margin:"4px auto 2px", fontWeight:700, fontSize:14, color:darkMode?"#f1f5f9":"#111", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:90 }}>{sorted[0].displayName||sorted[0].email?.split("@")[0]}</p>
            ...
            <div style={{ textAlign:"center", flex:1 }}>
              ...
              <p style={{ margin:"4px auto 2px", fontWeight:700, fontSize:12, color:darkMode?"#f1f5f9":"#111", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:75 }}>{sorted[2].displayName||sorted[2].email?.split("@")[0]}</p>
      ...
      <ScrollReveal direction="up" delay={100}>
        <div style={{ display:"flex", gap:6, marginBottom:20, flexWrap:"wrap" }}>
          {TABS.map((tab)=>(
            <button key={tab.id} onClick={()=>setActiveTab(tab.id)} style={{ padding:"8px 16px", borderRadius:20, border:"none", cursor:"pointer", background:activeTab===tab.id?"#3b82f6":(darkMode?"#1e293b":"#f1f5f9"), color:activeTab===tab.id?"#fff":(darkMode?"#94a3b8":"#374151"), fontSize:12, fontWeight:600, transition:"all 0.2s" }}>
              {tab.label}
            </button>
          ))}
        </div>
              ...
              return (
                <div key={u.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 16px", borderRadius:12, background:isMe?(darkMode?"#1e3a5f":"#eff6ff"):(darkMode?"#1e293b":"#fff"), border:`1px solid ${isMe?"#3b82f6":(darkMode?"#334155":"#e5e7eb")}`, transition:"all 0.2s" }}>
                  ...
                  <div style={{ width:80, display:"flex", flexDirection:"column", alignItems:"flex-end", gap:3 }}>
                    <span style={{ fontWeight:700, fontSize:13, color:i===0?"#f59e0b":i===1?"#94a3b8":i===2?"#cd7f32":"#3b82f6" }}>{valueLabel(u)}</span>
