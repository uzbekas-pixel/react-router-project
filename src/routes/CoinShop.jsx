...
const CoinShop = ({ darkMode, showToast }) => {
  ...
  const buyItem = async (item) => {
    ...
    if (notBuyable) { showToast && showToast("Bu mahsulot sizda bor!", "error"); return; }
    if (coins < item.price) { showToast && showToast("Yetarli coin yo'q!", "error"); return; }
    ...
    try {
      ...
      const newHistory = [{ id:item.id, name:item.name, icon:item.icon, price:item.price, date:new Date().toLocaleDateString("uz") }, ...(w.history||[])].slice(0,20);

      await setDoc(wRef, { coins:(w.coins||0)-item.price, owned:newOwned, history:newHistory, updatedAt:serverTimestamp() });
      ...
      showToast && showToast(`✅ "${item.name}" sotib olindi!`, "success");
    } catch (err) { console.error(err); showToast && showToast("Xatolik!", "error"); }
    setBuying(null);
  };

  const convertXpToCoins = async () => {
    if (!user || converting || xp < XP_PER_COIN) return;
    const amount = Math.min(convertAmount, Math.floor(xp/XP_PER_COIN)*XP_PER_COIN);
    ...
    try {
      const sRef = doc(db, "users", user.uid, "data", "stats");
      const sSnap = await getDoc(sRef);
      await setDoc(sRef, { ...(sSnap.data()||{}), xp:(sSnap.data()?.xp||0)-amount });
      setXp((p) => p - amount);

      const wRef = doc(db, "users", user.uid, "data", "wallet");
      const wSnap = await getDoc(wRef);
      await setDoc(wRef, { ...(wSnap.data()||{}), coins:(wSnap.data()?.coins||0)+gained, updatedAt:serverTimestamp() });

      showToast && showToast(`✅ ${amount} XP → ${gained} Coin!`, "success");
    } catch (err) { console.error(err); showToast && showToast("Xatolik!", "error"); }
    setConverting(false);
  };
            ...
            {filtered.map((item) => {
              ...
              return (
                <div key={item.id} style={{ padding:"18px 16px", borderRadius:16, background:darkMode?"#1e293b":"#fff", border:`1px solid ${noBuy?item.color+"66":(darkMode?"#334155":"#e5e7eb")}`, position:"relative" }}>
                  {noBuy && <span style={{ position:"absolute", top:10, right:10, background:"#10b981", color:"#fff", fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:10 }}>✓ Bor</span>}
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    ...
                    <button onClick={() => buyItem(item)} disabled={isBuying || noBuy}
                      ...
                      {isBuying ? <div style={{ width:14, height:14, borderRadius:"50%", border:"2px solid #fff", borderTopColor:"transparent", animation:"spin 0.8s linear infinite" }}/> : noBuy ? <><LuCheck size={14}/> Bor</> : canBuy ? <><LuShoppingBag size={14}/> Ol</> : <><LuLock size={14}/> Yetmaydi</>}
                    </button>
                  </div>
      ...
      {tab === "convert" && (
        <ScrollReveal direction="up" delay={100}>
          <div style={{ padding:"24px", borderRadius:16, background:darkMode?"#1e293b":"#fff", border:`1px solid ${darkMode?"#334155":"#e5e7eb"}`, marginBottom:16 }}>
            ...
            <input type="range" min={XP_PER_COIN} max={Math.max(XP_PER_COIN, Math.floor(xp/XP_PER_COIN)*XP_PER_COIN)} step={XP_PER_COIN} value={convertAmount} onChange={(e) => setConvertAmount(Number(e.target.value))} style={{ width:"100%", accentColor:"#f59e0b", marginBottom:8 }}/>
            ...
            <button onClick={convertXpToCoins} disabled={converting||xp<XP_PER_COIN}
              ...
              {converting ? <><div style={{ width:16, height:16, borderRadius:"50%", border:"2px solid #fff", borderTopColor:"transparent", animation:"spin 0.8s linear infinite" }}/> Konvertatsiya...</> : <><LuCoins size={18}/> {convertAmount} XP → {Math.floor(convertAmount/XP_PER_COIN)} Coin</>}
            </button>
          </div>
