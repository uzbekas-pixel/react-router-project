...
const VideoAvatarModal = ({ darkMode, onClose, onSave, showToast, t }) => {
  ...
  return (
    <div className="fixed inset-0 z-1000 flex items-center justify-center p-6 backdrop-blur-md bg-black/40 animate-in fade-in duration-300">
      <div className={`w-full max-w-md p-8 rounded-[2.5rem] border shadow-2xl transition-all duration-500 scale-in-center ${
...
const CoinShop = ({ darkMode, showToast }) => {
  ...
  const SHOP_ITEMS = useMemo(() => [
    { id:"xp_boost_2x",     category:"boost",    icon:<LuZap />,         name:t.itemXpBoost2xName,         desc:t.itemXpBoost2xDesc,                            price:50,  color:"#f59e0b" },
    { id:"xp_boost_3x",     category:"boost",    icon:<LuRocket />,      name:t.itemXpBoost3xName,         desc:t.itemXpBoost3xDesc,                            price:120, color:"#ef4444" },
    { id:"streak_shield",   category:"boost",    icon:<LuShield />,      name:t.itemStreakShieldName,       desc:t.itemStreakShieldDesc,                   price:30,  color:"#3b82f6" },
    { id:"hint_pack",       category:"boost",    icon:<LuLightbulb />,   name:t.itemHintPackName,            desc:t.itemHintPackDesc,                   price:40,  color:"#8b5cf6" },
    { id:"daily_double",    category:"boost",    icon:<LuTarget />,      name:t.itemDailyDoubleName,         desc:t.itemDailyDoubleDesc,             price:70,  color:"#06b6d4" },
    { id:"extra_lives",     category:"boost",    icon:<LuHeart />,       name:t.itemExtraLivesName,          desc:t.itemExtraLivesDesc,                    price:60,  color:"#ef4444" },
    { id:"xp_pack_500",     category:"xp",       icon:<LuStar />,        name:t.itemXpPack500Name,          desc:t.itemXpPack500Desc,                          price:100, color:"#10b981" },
    { id:"xp_pack_1000",    category:"xp",       icon:<LuSparkles />,    name:t.itemXpPack1000Name,         desc:t.itemXpPack1000Desc,                         price:180, color:"#f59e0b" },
    { id:"theme_neon",      category:"theme",    icon:<LuPalette />,     name:t.itemThemeNeonName,           desc:t.itemThemeNeonDesc,                price:100, color:"#06b6d4" },
    { id:"theme_gold",      category:"theme",    icon:<LuSparkles />,    name:t.itemThemeGoldName,           desc:t.itemThemeGoldDesc,               price:120, color:"#d97706" },
    { id:"theme_dracula",   category:"dev",      icon:<LuGhost />,       name:t.itemThemeDraculaName,        desc:t.itemThemeDraculaDesc,     price:200, color:"#8b5cf6" },
    { id:"badge_champion",  category:"badge",    icon:<LuTrophy />,      name:t.itemBadgeChampionName,       desc:t.itemBadgeChampionDesc,                price:150, color:"#f59e0b" },
    { id:"badge_pro",       category:"badge",    icon:<LuGem />,         name:t.itemBadgeProName,            desc:t.itemBadgeProDesc,                     price:200, color:"#8b5cf6" },
    { id:"snippet_slot_5",  category:"dev",      icon:<LuFolder />,      name:t.itemSnippetSlot5Name,      desc:t.itemSnippetSlot5Desc,         price:100, color:"#10b981" },
    { id:"name_gold",       category:"identity", icon:<LuSparkles />,    name:t.itemNameGoldName,            desc:t.itemNameGoldDesc,          price:80,  color:"#d97706", nameColor:"gold" },
    { id:"name_neon_blue",  category:"identity", icon:<LuGem />,         name:t.itemNameNeonBlueName,        desc:t.itemNameNeonBlueDesc,              price:80,  color:"#06b6d4", nameColor:"neon_blue" },
    { id:"name_neon_green", category:"identity", icon:<LuLeaf />,        name:t.itemNameNeonGreenName,      desc:t.itemNameNeonGreenDesc,             price:80,  color:"#10b981", nameColor:"neon_green" },
    { id:"name_purple",     category:"identity", icon:<LuSparkles />,       name:t.itemNamePurpleName,         desc:t.itemNamePurpleDesc,                     price:80,  color:"#8b5cf6", nameColor:"purple" },
    { id:"name_rose",       category:"identity", icon:<LuFlower />,       name:t.itemNameRoseName,          desc:t.itemNameRoseDesc,                  price:80,  color:"#f43f5e", nameColor:"rose" },
    { id:"name_rainbow",    category:"identity", icon:<LuSparkles />,    name:t.itemNameRainbowName,          desc:t.itemNameRainbowDesc,           price:200, color:"#6366f1", nameColor:"rainbow" },
    { id:"video_avatar",    category:"identity", icon:<LuVideo />,       name:t.itemVideoAvatarName,         desc:t.itemVideoAvatarDesc, price:300, color:"#ef4444" },
  ], [t]);
  ...
  const buyItem = async (item) => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        const newHistory = [
          { id:item.id, name:item.name, icon:item.icon, price:item.price, date:new Date().toLocaleDateString(t.lang === "uz" ? "uz-UZ" : "en-US") },
          ...(w.history || []),
        ].slice(0, 20);
  ...
  const handleVideoAvatarSave = async (url) => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        const newHistory = [
          { id:"video_avatar", name:t.itemVideoAvatarName, icon:<LuVideo />, price:300, date:new Date().toLocaleDateString(t.lang === "uz" ? "uz-UZ" : "en-US") },
          ...(w.history || []),
        ].slice(0, 20);
    ...
    <div className="w-full max-w-7xl mx-auto px-6 py-12 md:py-20 lg:py-24">
      ...
      <ScrollReveal direction="up">
        <div className="flex flex-wrap items-center justify-between gap-8 mb-12">
          <div>
            <span className="inline-block px-4 py-1.5 mb-6 text-xs font-bold tracking-widest uppercase rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/20">
              <LuCoins className="inline mr-2" /> {t.shopBadge || t.catDev}
            </span>
            <h2 className={`text-4xl md:text-5xl font-black tracking-tight ${darkMode ? "text-white" : "text-slate-900"}`}>
              {t.shopTitle}
            </h2>
            <p className="text-slate-500 text-sm font-medium mt-2">{t.shopSub}</p>
          </div>

          <div className="flex gap-6">
            {[
              { icon:<LuCoins className="text-amber-500" />, label:t.coinsLabel, value:coins, color:"amber" },
              { icon:<LuStar className="text-blue-500" />,  label:t.xpLabel,      value:xp.toLocaleString(), color:"blue" },
            ].map((s, i) => (
              <div key={i} className={`p-6 rounded-3xl border transition-all duration-500 ${
                darkMode ? "bg-slate-900/40 border-white/5" : "bg-white border-slate-100 shadow-xl shadow-slate-100/50"
              }`}>
                <div className="flex items-center gap-3 mb-2">
                  <span className={`p-2 rounded-xl bg-current/10 ${s.color === "amber" ? "text-amber-500" : "text-blue-500"}`}>{s.icon}</span>
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{s.label}</span>
                </div>
                <div className={`text-3xl font-black tabular-nums ${darkMode ? "text-white" : "text-slate-900"}`}>{s.value}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {(activeNameColor !== "default" || videoAvatarUrl) && (
          <div className={`p-8 rounded-[2.5rem] border mb-12 flex flex-wrap items-center gap-10 transition-all duration-500 ${
            darkMode ? "bg-slate-900/40 border-white/5" : "bg-white border-slate-100 shadow-xl shadow-slate-100"
          }`}>
            <div className="relative">
              {videoAvatarUrl ? (
                <div className="w-20 h-20 rounded-3xl overflow-hidden shadow-2xl ring-4 ring-red-500/20">
                  <video src={videoAvatarUrl} className="w-full h-full object-cover" muted autoPlay loop />
                </div>
              ) : (
                <div className="w-20 h-20 rounded-3xl bg-indigo-600 flex items-center justify-center text-white text-3xl font-black shadow-2xl">
                  {user?.displayName?.[0]?.toUpperCase() || "?"}
                </div>
              )}
              <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-amber-500 rounded-xl flex items-center justify-center text-white shadow-lg border-2 border-white dark:border-slate-900">
                <LuSparkles size={16} />
              </div>
            </div>
            <div className="flex-1">
              <span className={`text-2xl font-black tracking-tight mb-2 block ${darkMode ? "text-white" : "text-slate-900"}`} style={NAME_COLORS[activeNameColor]?.style}>
        ...
        <div className="flex gap-2 p-1.5 rounded-2xl bg-slate-900/10 dark:bg-slate-900/40 border dark:border-white/5 backdrop-blur-md mb-12">
          {[
            { id:"shop",    label:t.shopTabShop,    icon:<LuShoppingBag size={15}/> },
            { id:"convert", label:t.shopTabConvert, icon:<LuCoins size={15}/>       },
            { id:"history", label:t.shopTabHistory, icon:<LuHistory size={15}/>     },
          ].map((item) => (
            <button 
              key={item.id} 
              ...
              className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-xl text-sm font-black transition-all duration-500 ${
                tab === item.id 
                  ? "bg-amber-500 text-white shadow-xl shadow-amber-500/30 active:scale-95" 
          ...
          <div className="flex flex-wrap gap-2 mb-12">
            {CATEGORIES.map((c) => (
              <button 
                ...
                className={`flex items-center gap-3 px-6 py-3 rounded-xl border text-xs font-black transition-all duration-300 ${
                  category === c.id 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map((item) => {
              ...
              return (
                <div 
                  key={item.id} 
                  className={`p-8 rounded-[2.5rem] border transition-all duration-500 group flex flex-col relative ${
                    noBuy ? "opacity-60 grayscale-[0.5]" : ""
                  } ${
                    darkMode ? "bg-slate-900/40 border-white/5 hover:bg-slate-900/60" : "bg-white border-slate-100 shadow-lg shadow-slate-100/50 hover:shadow-2xl"
                  }`}
                >
                  {/* Badge Row */}
                  <div className="flex justify-between items-start mb-8">
                    <div 
                      className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shadow-xl transition-transform duration-500 group-hover:scale-110"
                      style={{ backgroundColor: `${item.color}15`, color: item.color }}
                    >
                      {item.icon}
                    </div>
                    {noBuy && (
                      <span className="px-3 py-1 rounded-lg bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase tracking-widest">
                        {isActiveNameItem ? "Active" : t.owned}
                      </span>
                    )}
                  </div>

                  <h3 className={`text-xl font-black mb-3 ${darkMode ? "text-white" : "text-slate-900"}`}>{item.name}</h3>
                  <p className="text-slate-500 text-sm font-medium leading-relaxed mb-8 flex-1">{item.desc}</p>

                  {item.nameColor && (
                    <div className={`p-4 rounded-2xl mb-8 text-center text-lg font-black transition-all ${
                  ...
                  {item.id==="video_avatar" && videoAvatarUrl && isOwned && (
                    <div className="relative h-24 rounded-2xl overflow-hidden mb-8 ring-4 ring-red-500/20">
                      <video src={videoAvatarUrl} className="w-full h-full object-cover" muted autoPlay loop />
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-2xl font-black tabular-nums text-amber-500 leading-none">{item.price}</span>
                    </div>
                    <button 
                      ...
                      className={`flex-1 py-4 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 ${
                        noBuy 
                          ...
                          : canBuy 
                            ? "bg-amber-600 text-white shadow-xl shadow-amber-600/30 hover:bg-amber-500 active:scale-95"
                            : "bg-slate-500/10 text-slate-500/60"
                      }`}
                    >
                      {isBuying ? <LuLoader className="animate-spin" /> : noBuy ? <LuCheck size={14} /> : canBuy ? t.get : <LuLock size={14} />}
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className={`flex items-center gap-6 p-5 rounded-4xl border transition-all hover:scale-[1.02] ${
                    ...
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl bg-opacity-10`} style={{ backgroundColor: `${item.color === 'emerald' ? '#10b981' : item.color === 'blue' ? '#3b82f6' : '#f59e0b'}15`, color: item.color === 'emerald' ? '#10b981' : item.color === 'blue' ? '#3b82f6' : '#f59e0b' }}>
                      {item.icon}
                    </div>
