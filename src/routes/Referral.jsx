...
const REFERRAL_XP = 100; // Taklif qiluvchiga beriladigan XP
const REFERRED_XP = 50;  // Yangi foydalanuvchiga beriladigan XP
...
const Referral = ({ darkMode, showToast }) => {
  ...
  // ── Referral code yaratish yoki yuklash ───────────────────────────────────
  useEffect(() => {
    if (!user) return;
    const load = async () => {
      setLoading(true);
  ...
  // ── Referral code qo'llash ────────────────────────────────────────────────
  const applyCode = async () => {
    if (!user || !inputCode.trim()) return;
    const code = inputCode.trim().toUpperCase();

    if (code === referralCode) {
      showToast && showToast("O'z kodingizni ishlata olmaysiz!", "error");
      return;
    }
    if (alreadyUsed) {
      showToast && showToast("Siz allaqachon referral kod ishlatgansiz!", "error");
      return;
    }
    ...
    try {
      ...
      if (snap.empty) {
        showToast && showToast("Kod topilmadi!", "error");
        setApplying(false);
        return;
      }
      ...
      await updateDoc(doc(db, "users", ownerData.uid, "data", "stats"), {
        xp: increment(REFERRAL_XP),
      }).catch(async () => {
        await setDoc(doc(db, "users", ownerData.uid, "data", "stats"), {
          xp: REFERRAL_XP, streak: 0,
        });
      });

      // Kod egasining referral doc yangilash
      ...
      setAlreadyUsed(true);
      setInputCode("");
      showToast && showToast(`🎉 +${REFERRED_XP} XP qo'shildi! Kod egasi ham +${REFERRAL_XP} XP oldi!`, "success");

      // Bildirishnoma yuborish — kod egasiga
      await setDoc(
  ...
  const copyCode = () => {
    ...
    setCopied(true);
    showToast && showToast("Kod nusxalandi! 📋", "success");
    setTimeout(() => setCopied(false), 2000);
  };

  // ── Ulashish ──────────────────────────────────────────────────────────────
  const shareCode = () => {
    const text = `Uzbekas Pixel platformasiga qo'shiling! Mening kodim: ${referralCode}\n+${REFERRED_XP} XP bonus olasiz 🎁\nhttps://react-router-project-l6dpclkou-uzbekas-pixels-projects.vercel.app`;
    if (navigator.share) {
      navigator.share({ title: "Uzbekas Pixel", text });
    } else {
      navigator.clipboard.writeText(text);
      showToast && showToast("Havola nusxalandi!", "success");
    }
  };
      ...
      <ScrollReveal direction="up">
        ...
        <div style={{ padding: "24px", borderRadius: 16, background: "linear-gradient(135deg, #3b82f611, #8b5cf611)", border: "2px solid #3b82f644", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <button onClick={copyCode} style={{ padding: "10px 16px", borderRadius: 10, border: "none", background: copied ? "#10b981" : "#3b82f6", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, transition: "all 0.2s" }}>
                {copied ? <><LuCheck size={16} /> Nusxalandi</> : <><LuCopy size={16} /> Nusxalash</>}
              </button>
              <button onClick={shareCode} style={{ padding: "10px 16px", borderRadius: 10, border: `1px solid ${darkMode ? "#334155" : "#e5e7eb"}`, background: "transparent", color: darkMode ? "#f1f5f9" : "#374151", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <div style={{ padding: "24px", borderRadius: 16, background: darkMode ? "#1e293b" : "#fff", border: `1px solid ${darkMode ? "#334155" : "#e5e7eb"}`, marginBottom: 24 }}>
          <p style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 700, color: darkMode ? "#f1f5f9" : "#111", display: "flex", alignItems: "center", gap: 6 }}>
            <LuZap size={16} className="text-yellow-400" /> Do'stingiz kodini kiriting
          </p>
        ...
        {referredUsers.length > 0 && (
          <div>
            <h3 style={{ margin: "0 0 14px", fontSize: 16, fontWeight: 700, color: darkMode ? "#f1f5f9" : "#111", display: "flex", alignItems: "center", gap: 8 }}>
              <LuUsers size={18} style={{ color: "#3b82f6" }} />
              Sizning kodingizni ishlatganlar ({referredUsers.length})
            </h3>
