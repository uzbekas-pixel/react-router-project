...
const WordsBox = ({ words, currentIndex, input, getLetterColor, darkMode, onFocus }) => {
  ...
  React.useLayoutEffect(() => {
    if (activeRef.current && containerRef.current) {
      setOffsetY(activeRef.current.offsetTop);
    }
  }, [currentIndex]);

  return (
    <div onClick={onFocus} ref={containerRef}
      style={{ height:112, overflow:"hidden", borderRadius:16, padding:"12px 16px", marginBottom:16, cursor:"text", userSelect:"none", position:"relative", background:darkMode?"#1e293b":"#fff", border:`1px solid ${darkMode?"#334155":"#e5e7eb"}` }}>
      <div style={{ display:"flex", flexWrap:"wrap", gap:"6px 10px", transform:`translateY(${-offsetY}px)`, transition:"transform 0.15s ease" }}>
        {words.map((word, wi) => (
          <span key={wi} ref={wi === currentIndex ? activeRef : null}
            style={{ padding:"2px 3px", borderRadius:4, fontSize:19, fontFamily:"monospace", lineHeight:"32px", background:wi===currentIndex?(darkMode?"#334155":"#dbeafe"):"transparent", outline:wi===currentIndex?"2px solid #3b82f6":"none", opacity:wi<currentIndex?0.3:1 }}>
      ...
      <div style={{ position:"absolute", bottom:0, left:0, right:0, height:28, background:`linear-gradient(transparent, ${darkMode?"#1e293b":"#fff"})`, pointerEvents:"none", borderRadius:"0 0 16px 16px" }} />
    </div>
  );
};
...
const Tournament = ({ darkMode, showToast }) => {
  ...
  // Typing tugaganda natija saqlash
useEffect(() => {
  if (finished && tab === "typing") {
    const wpm = Math.round((correctCount / 60) * 60);
    if (wpm > 0) {
      setTimeout(() => saveResult(wpm, "typing"), 0); // ← setTimeout qo'shildi
    }
  }
}, [finished, tab, correctCount, saveResult]);
    ...
    <div style={{ width:"100%", maxWidth:800, margin:"0 auto", padding:"40px 16px 80px" }}>
      <ScrollReveal direction="up">
        ...
        <div style={{ display:"flex", gap:8, marginBottom:16 }}>
          {[{id:"typing",label:"Typing",icon:<LuKeyboard size={16}/>},{id:"quiz",label:"Quiz",icon:<LuTarget size={16}/>}].map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ flex:1, padding:"12px 0", borderRadius:12, border:"none", background:tab===t.id?"#3b82f6":(darkMode?"#1e293b":"#f1f5f9"), color:tab===t.id?"#fff":(darkMode?"#94a3b8":"#374151"), fontSize:14, fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, transition:"all 0.2s" }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
  ...
  if (screen === "typing") return (
    <div style={{ width:"100%", maxWidth:760, margin:"0 auto", padding:"32px 16px 80px" }}>
      ...
      <WordsBox words={words} currentIndex={currentIndex} input={input} getLetterColor={getLetterColor} darkMode={darkMode} onFocus={() => inputRef.current?.focus()}/>
      ...
      <div style={{ width:"100%", maxWidth:640, margin:"0 auto", padding:"32px 16px 80px" }}>
        ...
        {selected !== null && (
          <button onClick={() => handleNextQ()}
            style={{ marginTop:16, width:"100%", padding:"14px 0", background:"#3b82f6", color:"#fff", border:"none", borderRadius:12, fontSize:15, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
