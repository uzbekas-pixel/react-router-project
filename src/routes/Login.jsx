...
import { githubProvider } from "../firebase/config";
...
const Login = ({ darkMode, showToast, showConfetti }) => {
  ...
  try {
    ...
    showConfetti();
    showToast("GitHub orqali kirdingiz! 🎉", "success");
    navigate("/");
  } catch {
    showToast("GitHub kirish xatoligi!", "error");
  } finally {
    setLoading(false);
  }
};
    ...
    <div className={`page-transition min-h-[calc(100vh-64px)] flex items-center justify-center px-6 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
      <div className={`w-full max-w-md p-8 rounded-2xl shadow-xl ${darkMode ? "bg-slate-800" : "bg-white"}`}>
        ...
        <div className="text-center mb-8">
          <h2 className={`text-3xl font-extrabold mb-2 ${darkMode ? "text-white" : "text-gray-900"}`}>Kirish</h2>
          <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Hisobingizga kiring</p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Parol labelidan keyin: */}
          <div className="flex items-center justify-between mb-1">
            <label className={`text-xs font-semibold ${darkMode ? "text-gray-400" : "text-gray-600"}`}>Parol</label>
  ...
  GitHub bilan kirish
          ...
          <p className={`text-center text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
            Hisobingiz yo'qmi?{" "}
            <Link to="/register" className="text-blue-400 hover:underline font-semibold">Ro'yxatdan o'ting</Link>
          </p>
        </div>
      </div>
    </div>
