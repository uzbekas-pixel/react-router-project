...
const formatCountdown = (ms) => {
  ...
  if (days > 0) return `${days} kun ${pad(hours)}:${pad(minutes)}:${pad(seconds)}`;
...
const PixelChallenge = ({ darkMode, showToast }) => {
  ...
  const [activeTab, setActiveTab] = useState("vazifa"); 
  ...
  const fetchChallengeData = useCallback(async () => {
    try {
      ...
      showToast?.("Musobaqani yuklashda xatolik yuz berdi", "error");
    } finally {
      setLoading(false);
    }
  }, [user, showToast]);
  ...
  const handleSubmitWork = async (e) => {
    ...
    if (timeLeft <= 0) return showToast?.("Vaqt tugadi, topshira olmaysiz", "warning");
    if (!subForm.title || !subForm.repoUrl) return showToast?.("Sarlavha va Repo linki shart", "warning");
    ...
    try {
      const newSubData = {
        ...
        userId: user.uid,
        userName: user.displayName || "Noma'lum O'quvchi",
        title: subForm.title,
      ...
      showToast?.("Ilovingiz musobaqaga topshirildi! Omad!", "success");
      fetchChallengeData(); 
    } catch (err) {
        console.error("Topshirish xatosi:", err);
      showToast?.("Topshirishda xatolik yuz berdi", "error");
    } finally {
  ...
  const handleVote = async (subId) => {
    if (!user) return showToast?.("Ovoz berish uchun tizimga kiring", "warning");
    if (timeLeft <= 0) return showToast?.("Musobaqa tugadi, ovoz berib bo'lmaydi", "warning");

    const targetSub = submissions.find(s => s.id === subId);
    if (!targetSub) return;
    if (targetSub.userId === user.uid) return showToast?.("O'z ishingizga ovoz bera olmaysiz!", "warning");
    if (targetSub.votes && targetSub.votes.includes(user.uid)) return showToast?.("Bu ishga allaqachon ovoz bergansiz", "warning");
    ...
    try {
      ...
      showToast?.("Ovozingiz qabul qilindi!", "success");
      fetchChallengeData(); 
    } catch (err) {
        console.error("Ovoz berish xatosi:", err);
      showToast?.("Ovoz berishda xatolik yuz berdi", "error");
    } finally {
      setActionLoading(null);
    }
  };
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-20 md:py-32 gap-4">
       ...
       <p className="text-slate-500 text-sm md:text-base font-bold tracking-widest uppercase">Challenge yuklanmoqda...</p>
    </div>
  );
  ...
  if (!currentChallenge) return (
    <div className={`w-full max-w-7xl mx-auto px-4 py-16 text-center ${darkMode ? "text-slate-400" : "text-slate-500"}`}>
        ...
        <h2 className="text-xl md:text-2xl font-black mb-2">Hozircha musobaqalar yo'q</h2>
        <p className="text-sm md:text-base">Yangi xakatonlar tez orada e'lon qilinadi.</p>
    </div>
  );
        ...
        <div className="flex flex-col md:flex-row gap-5 md:gap-6 justify-between items-start md:items-center mb-8 md:mb-12">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
                ...
                <p className="text-slate-500 text-xs md:text-sm font-medium mt-1">G'oya, tezlik va mahorat musobaqasi (24 soat).</p>
            </div>
          </div>
          <div className={`w-full md:w-auto px-4 md:px-5 py-3 md:py-3.5 rounded-2xl border flex items-center gap-3 shadow-lg transition-colors duration-300 ${
            ...
            <div>
              <p className="text-[9px] md:text-[10px] text-slate-500 font-black uppercase tracking-widest mb-0.5">Mukofot jamg'armasi</p>
              <p className="text-lg md:text-xl font-black text-amber-500 leading-none">{currentChallenge.prize} <span className="text-xs md:text-sm">Tanga</span></p>
            </div>
          </div>
        </div>
      </ScrollReveal>
        ...
        <div className={`p-5 sm:p-8 md:p-10 rounded-2xl md:rounded-[2.5rem] border mb-8 md:mb-10 transition-colors duration-300 ${
            ...
            <div className={`flex flex-col lg:flex-row gap-6 md:gap-8 justify-between items-start lg:items-center mb-6 md:mb-8 pb-6 md:pb-8 border-b transition-colors ${darkMode ? "border-white/10" : "border-slate-100"}`}>
                ...
                <div className={`w-full lg:w-auto px-6 md:px-8 py-5 md:py-6 rounded-2xl border text-center min-w-0 md:min-w-[200px] shrink-0 transition-colors duration-300 ${
                    ...
                    <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest md:tracking-wider opacity-70 mb-1">Tugashiga qoldi</p>
                    <p className="text-xl sm:text-2xl md:text-3xl font-black font-mono leading-none">
                        {timeLeft <= 0 ? "TUGADI" : formatCountdown(timeLeft)}
                    </p>
                </div>
            </div>
            ...
            <div className={`flex flex-col sm:flex-row gap-2 mb-6 md:mb-8 p-1.5 rounded-2xl border backdrop-blur-md transition-colors duration-300 ${
              ...
              <button onClick={() => setActiveTab("vazifa")} className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 md:px-6 rounded-xl text-xs md:text-sm font-black transition-all whitespace-nowrap ${ activeTab === "vazifa" ? "bg-purple-600 text-white shadow-lg shadow-purple-600/30" : "text-slate-500 hover:text-purple-600 hover:bg-slate-800/5" }`}>
                 <LuAward size={16} className="md:w-5 md:h-5"/> Vazifa va Topshirish
              </button>
              <button onClick={() => setActiveTab("submissions")} className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 md:px-6 rounded-xl text-xs md:text-sm font-black transition-all whitespace-nowrap ${ activeTab === "submissions" ? "bg-purple-600 text-white shadow-lg shadow-purple-600/30" : "text-slate-500 hover:text-purple-600 hover:bg-slate-800/5" }`}>
                 <LuFileCode2 size={16} className="md:w-5 md:h-5"/> O'quvchilar ishlari ({submissions.length})
              </button>
            </div>
            ...
            {activeTab === "vazifa" && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10">
                    <div className={`prose prose-sm md:prose-base max-w-none transition-colors duration-300 ${darkMode ? "prose-invert text-slate-300" : "text-slate-700"}`}>
                        <h4 className={`text-lg md:text-xl font-black mb-3 md:mb-4 transition-colors ${darkMode ? "text-white" : "text-slate-900"}`}>Musobaqa Qoidalari:</h4>
                        <ul className="space-y-2.5 md:space-y-3 list-decimal list-inside font-medium text-sm md:text-base">
                            <li>Loyiha faqat <span className="font-bold text-purple-500">Tailwind CSS</span> yordamida dizayn qilinishi kerak.</li>
                            <li>Dinamik holatlar (tugmalar bosilishi) <span className="font-bold text-blue-500">React/JS</span> orqali qilinishi shart.</li>
                            <li>Kodni GitHub yoki CodePen'ga joylab, ssilkasini topshiring.</li>
                            <li>Live Demo bo'lishi ishning g'olib bo'lish ehtimolini oshiradi.</li>
                            <li>G'olib faqat <span className="font-bold">O'quvchilarning ovoz berishi</span> orqali shaffof aniqlanadi.</li>
                        </ul>
                    </div>
                    ...
                    {timeLeft <= 0 ? (
                        <div className={`p-6 md:p-8 text-center rounded-2xl border font-bold text-red-500 text-sm md:text-base ${darkMode ? "bg-red-500/10 border-red-500/20" : "bg-red-50 border-red-200"}`}>
                            ⏳ Ushbu musobaqa yakunlandi. Yangisini kuting!
                        </div>
                    ) : mySubmission ? (
                        <div className={`p-6 md:p-8 rounded-2xl md:rounded-3xl border text-center transition-colors duration-300 ${ darkMode ? "bg-emerald-950/30 border-emerald-500/40" : "bg-emerald-50 border-emerald-200 shadow-xl" }`}>
                            ...
                            <h4 className="text-lg md:text-xl font-black mb-1 md:mb-2 text-emerald-500">Ajoyib! Ishingiz qabul qilindi.</h4>
                            <p className="text-xs md:text-sm text-slate-500 mb-5 md:mb-6 px-2">Loyiha nomi: "{mySubmission.title}"</p>
                            <div className="flex flex-col sm:flex-row justify-center gap-2 md:gap-3">
                                <a href={mySubmission.repoUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 px-4 py-3 md:py-2.5 text-xs md:text-sm font-bold bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-colors shadow-lg w-full sm:w-auto"><LuFileCode2 size={16}/> Repo'ni ko'rish</a>
                                {mySubmission.demoUrl && <a href={mySubmission.demoUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 px-4 py-3 md:py-2.5 text-xs md:text-sm font-bold bg-blue-600 text-white rounded-xl hover:bg-blue-500 transition-colors shadow-lg w-full sm:w-auto"><LuLink2 size={16}/> Demo ko'rish</a>}
                            </div>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmitWork} className={`p-5 sm:p-6 md:p-8 rounded-2xl md:rounded-3xl border transition-colors duration-300 ${darkMode ? "bg-slate-900 border-white/5 shadow-2xl shadow-black/50" : "bg-slate-50 border-slate-100 shadow-xl"}`}>
                            <h4 className={`text-lg md:text-xl font-black mb-5 md:mb-6 transition-colors ${darkMode ? "text-white" : "text-slate-900"}`}>O'z ishingizni topshiring</h4>
                            <div className="space-y-4">
                                <div>
                                    <label className="text-[10px] md:text-xs font-black uppercase text-slate-500 tracking-wider mb-1.5 md:mb-2 block">Ilova nomi *</label>
                                    <input required type="text" value={subForm.title} onChange={e => setSubForm({...subForm, title: e.target.value})} placeholder="Masalan: Minimal Music Player" className={`w-full p-3 md:p-4 rounded-xl border text-sm outline-none transition-all duration-300 ${darkMode ? "bg-slate-800 border-white/10 text-white focus:border-purple-600" : "bg-white border-slate-200 focus:border-purple-600 text-slate-900"}`} />
                                </div>
                                <div>
                                    <label className="text-[10px] md:text-xs font-black uppercase text-slate-500 tracking-wider mb-1.5 md:mb-2 block">Kod Linki (Repo) *</label>
                                    <div className="relative">
                                        <LuFileCode2 className="absolute left-3.5 md:left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                                        <input required type="url" value={subForm.repoUrl} onChange={e => setSubForm({...subForm, repoUrl: e.target.value})} placeholder="GitHub yoki CodePen linki" className={`w-full pl-10 md:pl-12 p-3 md:p-4 rounded-xl border text-sm outline-none transition-all duration-300 ${darkMode ? "bg-slate-800 border-white/10 text-white focus:border-purple-600" : "bg-white border-slate-200 focus:border-purple-600 text-slate-900"}`} />
                                    </div>
                                </div>
                                <div>
                                    <label className="text-[10px] md:text-xs font-black uppercase text-slate-500 tracking-wider mb-1.5 md:mb-2 block">Live Demo (Ixtiyoriy)</label>
                                    <div className="relative">
                                        <LuLink2 className="absolute left-3.5 md:left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                                        <input type="url" value={subForm.demoUrl} onChange={e => setSubForm({...subForm, demoUrl: e.target.value})} placeholder="Vercel, Netlify yoki boshqa link" className={`w-full pl-10 md:pl-12 p-3 md:p-4 rounded-xl border text-sm outline-none transition-all duration-300 ${darkMode ? "bg-slate-800 border-white/10 text-white focus:border-purple-600" : "bg-white border-slate-200 focus:border-purple-600 text-slate-900"}`} />
                                    </div>
                                </div>
                                <button type="submit" disabled={actionLoading === "submit"} className="w-full py-3.5 md:py-4 mt-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-black rounded-xl shadow-xl shadow-purple-600/30 transition-all text-sm flex items-center justify-center gap-2 active:scale-95">
                                    {actionLoading === "submit" ? <LuInfo className="animate-spin" size={18} /> : <><LuSend size={18} /> Vazifani Topshirish</>}
                                </button>
                            </div>
                        </form>
                    )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    {submissions.length === 0 && (
                        <div className="col-span-full py-12 md:py-16 text-center text-slate-500 text-sm md:text-base">
                            Hozircha hech kim ish topshirmadi. Birinchi bo'ling!
                        </div>
                    )}
                          ...
                          darkMode ? "bg-slate-900 border-white/5 hover:border-purple-600/50" : "bg-white border-slate-100 hover:border-purple-400"
                            ...
                            <div className="flex justify-between items-start mb-3 md:mb-4">
                                <span className={`px-2.5 md:px-3 py-1 md:py-1.5 rounded-lg text-[9px] md:text-[10px] font-black uppercase flex items-center gap-1 md:gap-1.5 transition-colors ${
                                    ...
                                    {index === 0 ? <LuAward size={12} className="md:w-[14px] md:h-[14px]"/> : ""}{index + 1}-O'RIN
                                </span>
                            ...
                            <div className="flex items-center gap-2 mb-5 md:mb-6">
                               ...
                               <span className="text-[11px] md:text-xs font-bold text-slate-500 line-clamp-1">{sub.userName} {isOwner ? "(Siz)" : ""}</span>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-2 md:gap-3 mb-5 md:mb-6 mt-auto">
                                <a href={sub.repoUrl} target="_blank" rel="noreferrer" className={`flex-1 flex items-center justify-center gap-1.5 md:gap-2 text-[11px] md:text-xs font-bold py-2.5 md:py-3 rounded-xl transition-colors ${darkMode ? "bg-slate-800 text-white hover:bg-slate-700" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>
                                    <LuFileCode2 size={14} className="md:w-4 md:h-4"/> Repo
                                </a>
                                {sub.demoUrl && (
                                    <a href={sub.demoUrl} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-1.5 md:gap-2 text-[11px] md:text-xs font-bold py-2.5 md:py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-500 transition-colors shadow-lg shadow-blue-500/20">
                                        <LuLink2 size={14} className="md:w-4 md:h-4"/> Demo
                                    </a>
                                )}
                            </div>
                            ...
                            {!isOwner && timeLeft > 0 && (
                                <button onClick={() => handleVote(sub.id)} disabled={actionLoading === `vote_${sub.id}` || hasVoted} className={`w-full py-2.5 md:py-3 rounded-xl font-black text-xs md:text-sm flex items-center justify-center gap-2 border transition-all active:scale-95 disabled:opacity-100 ${hasVoted ? "bg-red-500/10 text-red-500 border-red-500/30 cursor-not-allowed" : "bg-red-500 text-white hover:bg-red-600 border-red-500 shadow-lg shadow-red-500/25"}`}>
                                    {actionLoading === `vote_${sub.id}` ? <LuInfo className="animate-spin" size={16} /> : hasVoted ? <><LuCheck size={16} /> Ovoz berildi</> : <><LuHeart size={16} /> Ovoz berish</>}
                                </button>
                            )}
                        </div>
                        )
