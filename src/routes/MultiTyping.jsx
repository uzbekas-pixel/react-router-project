...
const MultiTyping = ({ darkMode, showToast }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "multiTyping", roomId), (snap) => {
      ...
      if (
        (data.status === "countdown" || data.status === "playing") &&
        screen !== "playing" &&
        screen !== "finished"
      ) {
        setScreen("playing");
  ...
  const joinRoom = async () => {
    ...
    if (!snap.exists()) {
      showToast(t.roomNotFound, "error");
      return;
    }
    const data = snap.data();
    if (data.guest) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-3xl">
        ...
        {screen === "lobby" && (
          <div
            className={`rounded-2xl p-8 shadow-xl ${darkMode ? "bg-slate-800" : "bg-white"}`}
          >
           <h1 className={`text-2xl font-extrabold mb-2 text-center flex items-center justify-center gap-3 ${darkMode ? "text-white" : "text-gray-900"}`}>
  <LuSwords className="text-blue-400" /> {t.multiTitle}
</h1>
            <p
              className={`text-sm text-center mb-6 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
            >
              {t.multiSub}
            </p>
  ...
  className="w-full py-4 bg-blue-500 hover:bg-blue-400 text-white font-semibold rounded-xl transition text-lg flex items-center justify-center gap-2">
  <LuUsers size={20} /> {t.createRoom}
</button>
  ...
  <LuArrowRight size={16} /> {t.joinRoom}
</button>
              </div>
              <p
  ...
  <LuLogOut size={14} /> {t.leaveGame}
            ...
            {/* Progress barlar */}
            <div
              className={`rounded-2xl p-4 mb-4 shadow ${darkMode ? "bg-slate-800" : "bg-white"}`}
            >
              <div className="mb-3">
                ...
                <div
                  className={`h-3 rounded-full overflow-hidden ${darkMode ? "bg-slate-700" : "bg-gray-100"}`}
                >
                  <div
                ...
                {words.map((word, wi) => (
                  ...
                  >
                    {word.split("").map((letter, li) => (
                      <span
                        key={li}
                        className={`transition-colors ${getLetterClass(wi, li)}`}
                      >
  ...
  {iWon ? <LuTrophy className="text-yellow-400" size={56} /> : <span>😢</span>}
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <div
                      className={`rounded-xl p-4 ${darkMode ? "bg-slate-700" : "bg-gray-50"}`}
                    >
  ...
  className="px-8 py-3 bg-blue-500 hover:bg-blue-400 text-white font-semibold rounded-xl transition flex items-center gap-2 mx-auto">
  <LuArrowRight size={16} /> {t.playAgain}
</button>
                </>
              );
            })()}
