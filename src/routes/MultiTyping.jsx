...
const MultiTyping = ({ darkMode, showToast }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "multiTyping", roomId), (snap) => {
      ...
      if (data.status === "countdown" && screen !== "playing") {
        setScreen("playing");
        setWords(data.words || []);
        startCountdown();
      }
  ...
  const createRoom = async () => {
    ...
    const wordsArr = generateWords(40);
  ...
  const handleInput = async (e) => {
    if (!started || screen !== "playing") return;
    const val = e.target.value;

    if (val.endsWith(" ")) {
      const typed = val.trim();
      if (typed !== words[currentIndex]) return;
      ...
      const elapsed = roomData?.startTime
        ? (Date.now() - roomData.startTime) / 60000
        : 1;
      const wpm = Math.round(newIndex / Math.max(elapsed, 0.1));
      ...
      <div className="w-full max-w-3xl">
          ...
          <div className={`rounded-2xl p-8 shadow-xl ${darkMode ? "bg-slate-800" : "bg-white"}`}>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-2">
                <input
                  type="text"
                  placeholder="Room ID kiriting (masalan: ABC123)"
                  value={joinInput}
        ...
        {screen === "waiting" && (
          <div className={`rounded-2xl p-8 shadow-xl text-center ${darkMode ? "bg-slate-800" : "bg-white"}`}>
            ...
            <button
              onClick={() => { navigator.clipboard.writeText(roomId); showToast("Nusxalandi!", "success"); }}
        ...
        {screen === "playing" && words.length > 0 && (
          <>
            ...
            <div
              ...
              className={`relative h-36 overflow-hidden rounded-2xl p-4 mb-4 cursor-text select-none shadow ${
