...
const Story = ({ darkMode, showToast }) => {
  ...
  const durationRef     = useRef(IMAGE_DURATION); // joriy story davomiyligi
  ...
  const handleUploadImage = async (e) => {
    ...
    if (!file) return;
    if (file.size > 20 * 1024 * 1024) { showToast?.(t.imageSizeError, "error"); return; }
    setUploading(true);
    try {
      ...
      if (!data.success) throw new Error("imgbb xato");
  ...
  const handleLike = async (e) => {
    ...
    try {
      ...
      setViewing((prev) =>
        prev.map((s, i) =>
          i === viewIndex
            ? { ...s, likes: liked ? s.likes.filter((id) => id !== user.uid) : [...(s.likes || []), user.uid] }
            : s
        )
      );
  ...
  const startProgressWithDuration = (userStories, index, duration) => {
    ...
    const steps    = 100;
    const interval = duration / steps; // har qadam ms
    ...
    timerRef.current = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        if (index + 1 < userStories.length) {
          ...
          setShowComments(false);
          openStoryAt(userStories, index + 1);
        } else {
          setViewing(null);
        }
      }
  ...
  const openStoryAt = (userStories, index) => {
    ...
    if (story.mediaType === "video") {
      // Video uchun — onLoadedMetadata da boshlaydi
      // Hozircha progress boshlamamiz, video yuklangandan keyin boshlanadi
      setProgress(0);
      pausedRef.current = false;
    } else {
      // Rasm uchun — 5 soniya
      startProgressWithDuration(userStories, index, IMAGE_DURATION);
    }
  };
  ...
  const handleVideoLoaded = () => {
    ...
    if (duration && !isNaN(duration) && isFinite(duration)) {
      startProgressWithDuration(viewing, viewIndex, duration * 1000);
    } else {
      // Duration aniqlanmasa 10 soniya default
      startProgressWithDuration(viewing, viewIndex, 10000);
    }
  };
  ...
  const handleVideoEnded = () => {
    ...
    if (viewIndex + 1 < viewing.length) {
      ...
      setShowComments(false);
      openStoryAt(viewing, viewIndex + 1);
    } else {
      setViewing(null);
    }
  };
  ...
  const toggleComments = (e) => {
    e.stopPropagation();
    if (!showComments) pauseStory(); else resumeStory();
    ...
    <div className={`page-transition w-full max-w-3xl mx-auto px-4 py-6 mt-10 ${darkMode ? "text-white" : "text-gray-900"}`}>
        ...
        <div className="flex flex-col items-center gap-2 shrink-0">
          <div className="relative">
            <button
              ...
              className={`relative w-16 h-16 rounded-full border-2 ${myStories.length > 0 ? "border-blue-500" : "border-dashed border-gray-400"} flex items-center justify-center overflow-hidden transition`}
            >
              {user?.photoURL
      ...
      {viewing && (
        <div className="fixed inset-0 z-9999 bg-black flex items-center justify-center">
          ...
          <div className="relative w-full max-w-[400px] h-full flex flex-col" style={{ maxHeight: "100dvh" }}>
            ...
            <div className="absolute top-8 left-4 right-4 z-20 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={() => { clearInterval(timerRef.current); setViewing(null); setShowComments(false); }}
            ...
            {/* Prev / Next zones */}
            <button
              className="absolute left-0 top-16 bottom-24 w-12 z-10"
              onClick={(e) => {
                e.stopPropagation();
                clearInterval(timerRef.current);
                if (viewIndex > 0) {
            ...
            />
            <button
              className="absolute right-0 top-16 bottom-24 w-12 z-10"
              onClick={(e) => {
                e.stopPropagation();
                clearInterval(timerRef.current);
                if (viewIndex + 1 < viewing.length) {
            ...
            <div className="absolute bottom-5 left-4 right-4 z-20 flex items-center gap-3">
              <button
                onClick={handleLike}
                className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-black/50 text-white text-sm font-semibold backdrop-blur-sm hover:bg-black/70 transition active:scale-95"
              >
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-white text-sm font-semibold backdrop-blur-sm transition active:scale-95 ${showComments ? "bg-blue-500/80" : "bg-black/50 hover:bg-black/70"}`}
              >
            ...
            {showComments && (
              <div
                className="absolute bottom-0 left-0 right-0 z-30 bg-black/90 backdrop-blur-md rounded-t-2xl flex flex-col"
                style={{ maxHeight: "55%" }}
                      ...
                      <div key={c.id} className="flex items-start gap-2 group">
                        ...
                        {(c.uid === user?.uid || isAdmin) && (
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 text-red-400 transition shrink-0 mt-0.5"
                          >
                            <LuTrash2 size={13} />
                          </button>
                        )}
