  ...
  import {
    MdOutlineLogout, MdOutlineLogin,
  } from "react-icons/md";
  ...
  const Navbar = ({ darkMode, setDarkMode, onNavClick }) => {
    ...
    const sidebarLinks = [
      ...
      ...(isAdmin ? [{ path: "/admin", label: t.adminBadge, icon: <LuShieldCheck className="text-yellow-400" /> }] : []),
      ...(isInstructor ? [{ path: "/instructor", label: "O'qituvchi Panel", icon: <LuBookOpen className="text-green-400" /> }] : []),
    ];
    ...
    getDoc(doc(db, "instructors", user.uid))
      .then((snap) => setIsInstructor(snap.exists() && snap.data().isInstructor));
  }, [user]);
      ...
      <>
        <nav className="bg-slate-800 shadow-lg fixed top-0 left-0 w-full z-50 ">
          <div className="flex items-center justify-between py-3 px-6">
            ...
            <div className="hidden md:flex items-center gap-2 ">
              ...
              <button onClick={() => setLang(lang === "en" ? "uz" : "en")}
                ...
                <LuLanguages className="text-blue-400 text-base" />
                {lang === "en" ? "UZ" : "EN"}
              </button>
          ...
          <div className={`md:hidden bg-slate-800 overflow-hidden transition-all duration-300 ${menuOpen ? "max-h-[700px] py-3" : "max-h-0"}`}>
            ...
            {/* Til */}
            <button onClick={() => setLang(lang === "en" ? "uz" : "en")}
              className="flex items-center gap-2 text-xs font-semibold text-white border border-slate-500 px-3 py-1.5 rounded-lg ml-8 mt-2">
        ...
        <>
          ...
          <div className={`hidden md:flex fixed top-0 right-0 h-full z-999 flex-col w-64 transition-transform duration-300 shadow-2xl ${
            darkMode ? "bg-slate-900 border-l border-slate-800" : "bg-white border-l border-gray-100"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700/50">
              ...
              <button onClick={() => setSidebarOpen(false)}
                className={`w-8 h-8 rounded-xl flex items-center justify-center transition ${
                  darkMode ? "bg-slate-700 text-white hover:bg-slate-600" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                }`}>
                <HiX />
              </button>
            </div>
