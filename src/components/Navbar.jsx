...
const Navbar = ({ darkMode, setDarkMode, onNavClick }) => {
  ...
  return (
    <nav className="bg-slate-800 shadow-lg fixed top-0 left-0 w-full z-50">
      <div className="flex items-center justify-between py-3 px-6 md:px-15">
  ...
  const labels = ["Home", "About", "Contact", "Products", "💻 Code", "🎮 Games", "💬"];
          ...
          {isAdmin && (
            <NavLink to="/admin" onClick={onNavClick}
              className={({ isActive }) =>
                `py-1 px-3 text-lg font-light rounded-2xl transition duration-300 ${
                  isActive ? "text-yellow-300 bg-slate-700" : "text-yellow-400 hover:text-yellow-300 hover:bg-slate-700"
                }`
              }>
              🛡️ Admin
            </NavLink>
