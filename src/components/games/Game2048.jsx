...
const moveBoard = (board, dir) => {
  ...
  if (dir === "left") {
    ...
    let transposed = newBoard[0].map((_, c) => newBoard.map(r => r[c]));
    transposed = process(transposed);
    newBoard = transposed[0].map((_, c) => transposed.map(r => r[c]));
  } else if (dir === "down") {
    let transposed = newBoard[0].map((_, c) => newBoard.map(r => r[c])).map(r => [...r].reverse());
    transposed = process(transposed);
    transposed = transposed.map(r => [...r].reverse());
    newBoard = transposed[0].map((_, c) => transposed.map(r => r[c]));
  }
...
const TILE_COLORS = {
  0:    { bg: "bg-slate-700/50", text: "" },
  2:    { bg: "bg-slate-500", text: "text-white" },
  4:    { bg: "bg-slate-400", text: "text-white" },
...
const Game2048 = ({ darkMode }) => {
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-[calc(100vh-130px)] px-4 py-6 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}>
      <h2 className={`text-2xl font-extrabold mb-4 ${darkMode ? "text-white" : "text-gray-900"}`}>
        🎯 2048
      </h2>
  ...
  <FaRedo /> {t.again}
      ...
      {/* Board */}
      <div
        className={`relative p-3 rounded-2xl select-none ${darkMode ? "bg-slate-800" : "bg-gray-200"}`}
        ...
        <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${SIZE}, 1fr)` }}>
          {board.map((row, r) =>
            row.map((val, c) => {
              ...
              return (
                <div key={`${r}-${c}`}
                  className={`w-16 h-16 md:w-20 md:h-20 rounded-xl flex items-center justify-center font-extrabold transition-all duration-100 ${bg} ${text} ${getFontSize(val)}`}>
                  {val !== 0 ? val : ""}
                </div>
              );
            })
