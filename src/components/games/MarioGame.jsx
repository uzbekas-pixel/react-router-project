...
const MarioGame = ({ darkMode }) => {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", minHeight:"calc(100vh-130px)", padding:"20px 16px 80px", background:darkMode?"#0f172a":"#1a1a2e" }}>
      <h2 style={{ color:"#ffd700", fontWeight:800, fontSize:24, marginBottom:8, textShadow:"2px 2px 0 #8b0000" }}>
        🍄 Super Mario
      </h2>
      ...
      {gameState==="playing" && (
        <div style={{ display:"flex", gap:20, marginBottom:8, color:"#fff", fontFamily:"monospace", fontSize:14, fontWeight:700 }}>
          ...
          <span>❤️ ×{displayLives}</span>
          <span>🌍 {displayLevel}-1</span>
        </div>
      )}
      ...
      <div style={{ position:"relative", borderRadius:8, overflow:"hidden", boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
        ...
        {gameState==="menu" && (
          <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:14 }}>
            ...
            <p style={{ color:"#fff", fontSize:13, margin:0 }}>PC: ← → harakat, ↑ / Space — sakrash</p>
            <p style={{ color:"rgba(255,255,255,0.6)", fontSize:12, margin:0 }}>Mushroom — kattalashish | Goomba ustidan bosish</p>
            <button onClick={startGame}
              style={{ marginTop:8, padding:"14px 44px", background:"#e8a020", color:"#fff", border:"none", borderRadius:12, fontSize:20, fontWeight:800, cursor:"pointer", boxShadow:"0 4px 0 #8b6020" }}>
              ▶ BOSHLASH
            </button>
            <button onClick={() => setShowLB(!showLB)}
              style={{ padding:"8px 22px", background:"transparent", color:"#ffd700", border:"2px solid #ffd700", borderRadius:10, fontSize:13, fontWeight:600, cursor:"pointer" }}>
              🏆 Leaderboard
            </button>
          </div>
        )}
        ...
        {gameState==="gameover" && (
          <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.82)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:14 }}>
            ...
            <button onClick={startGame}
              style={{ padding:"12px 32px", background:"#3b82f6", color:"#fff", border:"none", borderRadius:10, fontSize:16, fontWeight:700, cursor:"pointer" }}>
              🔄 Qayta
            </button>
        ...
        {gameState==="win" && (
          <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.82)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:14 }}>
            <div style={{ fontSize:64 }}>🏆</div>
            <h2 style={{ color:"#ffd700", fontSize:30, fontWeight:800, margin:0 }}>G'ALABA!</h2>
            <p style={{ color:"#fff", fontSize:16, margin:0 }}>Barcha 3 levelni tugatdingiz!</p>
            <p style={{ color:"#ffd700", fontSize:22, fontWeight:700, margin:0 }}>Score: {displayScore}</p>
            <button onClick={startGame}
              style={{ padding:"12px 32px", background:"#10b981", color:"#fff", border:"none", borderRadius:10, fontSize:16, fontWeight:700, cursor:"pointer" }}>
              🔄 Qaytadan
            </button>
          </div>
        )}
      ...
      {gameState==="playing" && (
        <div style={{ display:"flex", gap:10, marginTop:10, alignItems:"center" }}>
          <MBtn keysRef={keysRef} code="ArrowLeft"  label="◀"/>
          <MBtn keysRef={keysRef} code="ArrowRight" label="▶"/>
          <div style={{ width:20 }}/>
          <MBtn keysRef={keysRef} code="Space" label="🅰" style={{ background:"rgba(239,68,68,0.35)", borderColor:"#ef4444" }}/>
        </div>
      )}
